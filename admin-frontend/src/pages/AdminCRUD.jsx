...
function AdminCRUD() {
  ...
  const fetchCourses = async () => {
    try {
      const res = await fetch("http://localhost:3000/api/v1/course/preview");
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = "http://localhost:3000/api/v1/admin/course";
  ...
  const confirmDelete = async (courseId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/api/v1/admin/course/${courseId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
